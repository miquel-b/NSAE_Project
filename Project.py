...
#es el mateix pero aplicant las betas
import math
import matplotlib
from matplotlib import pyplot as plt
import numpy as np

def fractal(l, n, angle, beta): #per a que s'assembli mes a un triangle, n ha de ser gran
    x_dots = []
    y_up = []
    y_mid = []
    y_down = []
    ...
    # for i in range(1,n):
    #    plt.fill_between([l*((beta-1)/(beta))**(i)], [l*((beta-1)/(beta))**i], [0,(x_dots[i]*np.cos(angle))], [0,-(x_dots[i]*np.cos(angle))], color = 'black')
